...
export default function RichMenuImportDialog(
  ...
  const { data: remoteRichMenuList } = useSWR<{ richmenus: { [key: string]: string }[] }>(() => (currentAccount.channelAccessToken) ? `/api/v2/bot/richmenu/list` : null, url => axios.get(url, { headers: { Authorization: `Bearer ${currentAccount.channelAccessToken}` }}).then(({ data }) => data), { dedupingInterval: 5000 });
        ...
        <Button onClick={() => {
          ...
          if (tabIndex === "1" && remoteRichMenuList?.richmenus?.[selectedRichMenuIndex]?.richMenuId) {
            (async () => {
              ...
              const imageData = await axios.get<Blob>(`https://api-data.line.me/v2/bot/richmenu/${remoteRichMenuList.richmenus[selectedRichMenuIndex].richMenuId}/content`, { headers: { Authorization: `Bearer ${currentAccount.channelAccessToken}` }, responseType: "blob" });
